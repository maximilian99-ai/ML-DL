...
# python main.py "path" -a "mobilenet_v2"
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # train 데이터 설정
    train_transform = transforms.Compose([      # Augmentation이 적용되는 곳
            # transforms.Resize(224)
            transforms.RandomResizedCrop(224),  # 사이즈 조정
            transforms.RandomHorizontalFlip(),  # 랜덤 좌우 반전
            transforms.Pad(2),

            # Don't touch
            transforms.ToTensor(),              # (필수)학습할 수 있는 자료형으로 변경해줌
            normalize,                          # (필수)
        ])
